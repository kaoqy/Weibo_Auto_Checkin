...
function mockFetch(url) {
  ...
  const table = {
    ...
    '/api/logs/paginated': { ok: true, json: () => Promise.resolve({ items: mockLogs, total: mockLogs.length, has_more: false }) },
...
(async () => {
  ...
  console.log('— 导航 —');
  check('6 个导航项（含 1.0.0 超话）', window.document.querySelectorAll('.nav-item').length === 6);
  ...
  check('样式缓存版本为 1.0.0', pageHtml.includes('/style.css?v=1.0.0'));
  check('脚本缓存版本为 1.0.0', pageHtml.includes('/app.js?v=1.0.0'));
